...
from app.mixins import UidPrimaryModel, StatusMixin
...
class URL(UidPrimaryModel, StatusMixin):
...
class ExcelRow(UidPrimaryModel, StatusMixin):
...
class File(UidPrimaryModel, StatusMixin):
    template = models.ForeignKey(Template, on_delete=models.CASCADE, null=True)
    excel = models.FileField(upload_to="media/doc")
